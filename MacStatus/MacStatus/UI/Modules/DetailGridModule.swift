        ...
        VStack(alignment: .leading, spacing: 0) {
            VStack(spacing: 12) {
                ...
                HStack {
                    ...
                    Spacer()
                    detailItem(title: "Current", value: "\(batteryData.amperage) mA")
                }
            }
            ...
            if let adapter = batteryData.adapter {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack {
                        detailItem(title: "Design Power", value: "\(adapter.designWatts) W", width: 80)
                        Spacer()
                        if let profile = adapter.activeProfile {
                            detailItem(title: "Current PD", value: String(format: "%.1fV / %.1fA", profile.maxVoltage, profile.maxCurrent), width: 120)
                        } else {
                            detailItem(title: "Adapter Protocol", value: adapter.name, width: 120)
                        }
                    }
                    .padding(.horizontal, 4)
                }
